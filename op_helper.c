...
void do_store_vector_b(int addr, int data ,int i)                    //mari
{
    ...
    {
         t1 = 3 - addr; 
         env->vector_gpr[i].part[0] =  env->vector_gpr[i].part[0] & mask_m_b[t1];
         env->vector_gpr[i].part[0] =  env->vector_gpr[i].part[0] | (data<<(t1*8));
     }
     else if((addr>=4)&&(addr<8))
     {
          t = addr%4;
          t1 = 3 - t;
          env->vector_gpr[i].part[1] =  env->vector_gpr[i].part[1] & mask_m_b[t1];
          env->vector_gpr[i].part[1] =  env->vector_gpr[i].part[1] | (data<<(t1*8));    
      }
     else if((addr>=8)&&(addr<12))
     {
          t = addr%4;
          t1 = 3 - t;
          env->vector_gpr[i].part[2] =  env->vector_gpr[i].part[2] & mask_m_b[t1];
          env->vector_gpr[i].part[2] =  env->vector_gpr[i].part[2] | (data<<(t1*8));
      }
      else if((addr>=12)&&(addr<16))
     {
          t = addr%4;
          t1 = 3 - t;
          env->vector_gpr[i].part[3] =  env->vector_gpr[i].part[3] & mask_m_b[t1];
          env->vector_gpr[i].part[3] =  env->vector_gpr[i].part[3] | (data<<(t1*8));   
      }
...
void do_store_vector_h(int addr, int data ,int i)                    //mari
{
    ...
    {
         t1 = 2 - addr;
         env->vector_gpr[i].part[0] =  env->vector_gpr[i].part[0] & mask_m_h[t1];
         env->vector_gpr[i].part[0] =  env->vector_gpr[i].part[0] | (data<<(t1*8));
     }
     else if((addr>=4)&&(addr<8))
     {
          t = addr%4;
          t1 = 2 - t;
          env->vector_gpr[i].part[1] =  env->vector_gpr[i].part[1] & mask_m_h[t1];
          env->vector_gpr[i].part[1] =  env->vector_gpr[i].part[1] | (data<<(t1*8));    
      }
     else if((addr>=8)&&(addr<12))
     {
          t = addr%4;
          t1 = 2 - t;
          env->vector_gpr[i].part[2] =  env->vector_gpr[i].part[2] & mask_m_h[t1];
          env->vector_gpr[i].part[2] =  env->vector_gpr[i].part[2] | (data<<(t1*8));
      }
      else if((addr>=12)&&(addr<16))
     {
          t = addr%4;
          t1 = 2 - t;
          env->vector_gpr[i].part[3] =  env->vector_gpr[i].part[3] & mask_m_h[t1];
          env->vector_gpr[i].part[3] =  env->vector_gpr[i].part[3] | (data<<(t1*8));   
      }
}
void do_store_vector_w(int addr, int data ,int i)                    //mari
{
    ...
    if(addr<4)
    {
         env->vector_gpr[i].part[0] =  env->vector_gpr[i].part[0] & mask_m_w;
         env->vector_gpr[i].part[0] =  env->vector_gpr[i].part[0] | data;
     }
     else if((addr>=4)&&(addr<8))
     {
          env->vector_gpr[i].part[1] =  env->vector_gpr[i].part[1] & mask_m_w;
          env->vector_gpr[i].part[1] =  env->vector_gpr[i].part[1] | data;    
      }
     else if((addr>=8)&&(addr<12))
     {
          env->vector_gpr[i].part[2] =  env->vector_gpr[i].part[2] & mask_m_w;
          env->vector_gpr[i].part[2] =  env->vector_gpr[i].part[2] | data;
      }
      else if((addr>=12)&&(addr<16))
     {
          env->vector_gpr[i].part[3] =  env->vector_gpr[i].part[3] & mask_m_w;
          env->vector_gpr[i].part[3] =  env->vector_gpr[i].part[3] | data;   
      }
}
...
{ 
    ...
    {
         t1 = 3-addr; 
         data =  env->vector_gpr[i].part[0] & (~mask_m_b[t1]);
         data = data>>(t1*8);
     }
     else if((addr>=4)&&(addr<8))
     {
          t = addr%4;
          t1 = 3 - t;
          data =  env->vector_gpr[i].part[1] & (~mask_m_b[t1]);
          data = data>>(t1*8);
      }
     else if((addr>=8)&&(addr<12))
     {
          t = addr%4;
          t1 = 3 - t;
           data =  env->vector_gpr[i].part[2] & (~mask_m_b[t1]);
         data = data>>(t1*8);
      }
      else if((addr>=12)&&(addr<16))
     {
          t = addr%4;
          t1 = 3 - t;
          data =  env->vector_gpr[i].part[3] & (~mask_m_b[t1]);
          data = data>>(t1*8);
      }
     return data;
}
...
{ 
    ...
    {
         t1 = 2-addr; 
         data =  env->vector_gpr[i].part[0] & (~mask_m_h[t1]);
         data = data>>(t1*8);
     }
     else if((addr>=4)&&(addr<8))
     {
          t = addr%4;
          t1 = 2 - t;
          data =  env->vector_gpr[i].part[1] & (~mask_m_h[t1]);
          data = data>>(t1*8);
      }
     else if((addr>=8)&&(addr<12))
     {
          t = addr%4;
          t1 = 2 - t;
           data =  env->vector_gpr[i].part[2] & (~mask_m_h[t1]);
         data = data>>(t1*8);
      }
      else if((addr>=12)&&(addr<16))
     {
          t = addr%4;
          t1 = 2 - t;
          data =  env->vector_gpr[i].part[3] & (~mask_m_h[t1]);
          data = data>>(t1*8);
      }
     return data;
}
...
{ 
    ...
    if(addr<4)
    {
         data =  env->vector_gpr[i].part[0];
     }
     else if((addr>=4)&&(addr<8))
     {
          data =  env->vector_gpr[i].part[1];
      }
     else if((addr>=8)&&(addr<12))
     {
         data =  env->vector_gpr[i].part[2];
      }
      else if((addr>=12)&&(addr<16))
     {
          data =  env->vector_gpr[i].part[3];
      }
     return data;
}
...
void do_init_vec_l(int addr ,int j)        //mari
{
    ...
    switch(addr){
      case 0:
          env->vector_gpr[j].part[0] = 0x00010203;
          env->vector_gpr[j].part[1] = 0x04050607;
          env->vector_gpr[j].part[2] = 0x08090a0b;
          env->vector_gpr[j].part[3] = 0x0c0d0e0f;
          break;
      case 1:
          env->vector_gpr[j].part[0] = 0x01020304;
          env->vector_gpr[j].part[1] = 0x05060708;
          env->vector_gpr[j].part[2] = 0x090a0b0c;
          env->vector_gpr[j].part[3] = 0x0d0e0f10;
          break;
      case 2:
          env->vector_gpr[j].part[0] = 0x02030405;
          env->vector_gpr[j].part[1] = 0x06070809;
          env->vector_gpr[j].part[2] = 0x0a0b0c0d;
          env->vector_gpr[j].part[3] = 0x0e0f1011;
          break;
      case 3:
          env->vector_gpr[j].part[0] = 0X03040506;
          env->vector_gpr[j].part[1] = 0x0708090a;
          env->vector_gpr[j].part[2] = 0x0b0c0d0e;
          env->vector_gpr[j].part[3] = 0x0f101112;
          break;
     case 4:
          env->vector_gpr[j].part[0] = 0x04050607;
          env->vector_gpr[j].part[1] = 0x08090a0b;
          env->vector_gpr[j].part[2] = 0x0a0d0e0f;
          env->vector_gpr[j].part[3] = 0x10111213;
          break;
     case 5:
          env->vector_gpr[j].part[0] = 0x05060708;
          env->vector_gpr[j].part[1] = 0x090a0b0c;
          env->vector_gpr[j].part[2] = 0x0d0e0f10;
          env->vector_gpr[j].part[3] = 0x11121314;
          break;
     case 6:
          env->vector_gpr[j].part[0] = 0x06070809;
          env->vector_gpr[j].part[1] = 0x0a0b0c0d;
          env->vector_gpr[j].part[2] = 0x0e0f1011;
          env->vector_gpr[j].part[3] = 0x12131415;
          break;
     case 7:
          env->vector_gpr[j].part[0] = 0x0708090a;
          env->vector_gpr[j].part[1] = 0x0b0c0d0e;
          env->vector_gpr[j].part[2] = 0x0f101112;
          env->vector_gpr[j].part[3] = 0x13141516;
          break;
     case 8:
          env->vector_gpr[j].part[0] = 0x08090a0b;
          env->vector_gpr[j].part[1] = 0x0c0d0e0f;
          env->vector_gpr[j].part[2] = 0x10111213;
          env->vector_gpr[j].part[3] = 0x14151617;
          break;
     case 9:
          env->vector_gpr[j].part[0] = 0x090a0b0c;
          env->vector_gpr[j].part[1] = 0x0d0e0f10;
          env->vector_gpr[j].part[2] = 0x11121314;
          env->vector_gpr[j].part[3] = 0x15161718;
          break;
     case 10:
          env->vector_gpr[j].part[0] = 0x0a0b0c0d;
          env->vector_gpr[j].part[1] = 0x0e0f1011;
          env->vector_gpr[j].part[2] = 0x12131415;
          env->vector_gpr[j].part[3] = 0x16171819;
          break;
     case 11:
          env->vector_gpr[j].part[0] = 0x0b0c0d0e;
          env->vector_gpr[j].part[1] = 0x0f101112;
          env->vector_gpr[j].part[2] = 0x13141516;
          env->vector_gpr[j].part[3] = 0x1718191a;
          break;
     case 12:
          env->vector_gpr[j].part[0] = 0x0c0d0e0f;
          env->vector_gpr[j].part[1] = 0x10111213;
          env->vector_gpr[j].part[2] = 0x14151617;
          env->vector_gpr[j].part[3] = 0x18191a1b;
          break;
    case 13:
          env->vector_gpr[j].part[0] = 0x0d0e0f10;
          env->vector_gpr[j].part[1] = 0x11121314;
          env->vector_gpr[j].part[2] = 0x15161718;
          env->vector_gpr[j].part[3] = 0x191a1b1c;
          break;
    case 14:
          env->vector_gpr[j].part[0] = 0x0e0f1011;
          env->vector_gpr[j].part[1] = 0x12131415;
          env->vector_gpr[j].part[2] = 0x16171819;
          env->vector_gpr[j].part[3] = 0x1a1b1c1d;
          break;
    case 15:
          env->vector_gpr[j].part[0] = 0x0f101112;
          env->vector_gpr[j].part[1] = 0x13141516;
          env->vector_gpr[j].part[2] = 0x1718191a;
          env->vector_gpr[j].part[3] = 0x1b1c1d1e;
          break;
    default:
          break;        
    }
...
void do_init_vec_r(int addr ,int j)        //mari
{
    ...
    switch(addr){
      case 0:
          env->vector_gpr[j].part[0] = 0x10111213;
          env->vector_gpr[j].part[1] = 0x14151617;
          env->vector_gpr[j].part[2] = 0x18191a1b;
          env->vector_gpr[j].part[3] = 0x1c1d1e1f;
          break;
      case 1:
          env->vector_gpr[j].part[0] = 0x0f101112;
          env->vector_gpr[j].part[1] = 0x13141516;
     ...
     case 12:
          env->vector_gpr[j].part[0] = 0x04050607;
          env->vector_gpr[j].part[1] = 0x08090a0b;
          env->vector_gpr[j].part[2] = 0x0c0d0e0f;
          env->vector_gpr[j].part[3] = 0x10111213;
          break;
    case 13:
          env->vector_gpr[j].part[0] = 0x03040506;
    ...
    case 14:
          ...
          env->vector_gpr[j].part[1] = 0x06070809;
          env->vector_gpr[j].part[2] = 0x0a0b0c0d;
          env->vector_gpr[j].part[3] = 0x0e0f01011;
          break;
    case 15:
          env->vector_gpr[j].part[0] = 0x01020304;
          env->vector_gpr[j].part[1] = 0x05060708;
...
unsigned int shifting(unsigned int m)    //mari
{
	unsigned int a,b,c,n;
	a=(m&0x01f80000)<<7;  //get 7-12bit of the word
	b=(m&0x0000f800)<<10; //get 16-20bit of the word
	c=(m&0x000000f8)<<13; //get 24-28bit of the word
	n=a|b|c; //make n the half word we need
	return n;
}
//get the bits in the intervals of chain     //mari
unsigned int chooseInterval(unsigned int chain, unsigned int interval, unsigned int shiftLeft)
{
         unsigned int temp;
         temp = interval << shiftLeft; //shift left interval
         temp = temp & chain; //choose the bits in the intervals of vrc
         temp = temp >> shiftLeft; // necessary bits
         return temp;
}

//function EXTS with length of bits
signed int EXTS(unsigned int n, unsigned int number)    //mari
{
         bool sign;
         unsigned int temp;
         sign = n/(1<<(number-1));//find the sign bit
         if (sign)  {
              temp = (1<<number)-1;
              temp = ~temp;
              n = n|temp;
...
signed int ClampS(signed int a, signed int lowerLimit, signed int upperLimit)   //mari
{
         if (a>=0) 
         {
              if (a<upperLimit) return a;
              else return upperLimit;
         }
         else {
              if (a>lowerLimit) return a;
              else return lowerLimit;
         }
}
unsigned int ClampU(unsigned int a, unsigned int lowerLimit, unsigned int upperLimit)   //mari
{
         if (a>=0) 
         {
              if (a<upperLimit) return a;
...
void do_vector_pack_pixel(int j, int a, int b)    //mari
{
        int i;
        for (i=0;i<4;i++)
        env->vector_gpr[j].part[i] = 0;      //let vrt be zeros

	for (i=0;i<4;i++)
	{
	    env->vector_gpr[j].part[i/2] = env->vector_gpr[j].part[i/2]|(shifting(env->vector_gpr[a].part[i])>>16*(i%2));
	    env->vector_gpr[j].part[2+i/2] =env->vector_gpr[j].part[2+i/2]|(shifting(env->vector_gpr[b].part[i])>>16*(i%2));
    }
}
void do_vector_pack_shss(int j,int a,int b)   //mari
{
        signed int x,y;
	int i;
	for (i=0;i<8;i++)
        {   
	    x = chooseInterval(env->vector_gpr[a].part[i/2], 0xffff, 16-(i%2)*16);
            x = EXTS((unsigned)x,16);
            x = ClampS(x, -128, 127);
            env->vector_gpr[j].part[i/4] = (env->vector_gpr[j].part[i/4]<<8)|(x&0xff);
	    y = chooseInterval(env->vector_gpr[b].part[i/2], 0xffff, 16-(i%2)*16);
            y = EXTS((unsigned)y,16);
	    y = ClampS(y, -128, 127);
            env->vector_gpr[j].part[2+i/4] = (env->vector_gpr[j].part[2+i/4]<<8)|(y&0xff);
        }
}

void do_vector_pack_shus(int j,int a,int b)   //mari
{
        signed int x,y;
	int i;
	for (i=0;i<8;i++)
	    ...
	    x = chooseInterval(env->vector_gpr[a].part[i/2], 0xffff, 16-(i%2)*16);
            x = EXTS((unsigned)x,16);
            x = ClampU((unsigned)x, 0, 255);
            env->vector_gpr[j].part[i/4] = (env->vector_gpr[j].part[i/4]<<8)|(x&0xff);
	    y = chooseInterval(env->vector_gpr[b].part[i/2], 0xffff, 16-(i%2)*16);
            y = EXTS((unsigned)y,16);
	    y = ClampU((unsigned)y, 0, 255);
            env->vector_gpr[j].part[2+i/4] = (env->vector_gpr[j].part[2+i/4]<<8)|(y&0xff);
        }
}

void do_vector_pack_swss(int j,int a,int b)   //mari
{
        signed int x,y;
	int i;
	for (i=0;i<4;i++)
        {   
	    x = chooseInterval(env->vector_gpr[a].part[i], 0xffffffff, 0);
            x = ClampS(x, -32768, 32767);
            env->vector_gpr[j].part[i/2] = (env->vector_gpr[j].part[i/2]<<16)|(x&0xffff);
	    y = chooseInterval(env->vector_gpr[b].part[i], 0xffffffff, 0);
	    y = ClampS(y, -32768, 32767);
            env->vector_gpr[j].part[2+i/2] = (env->vector_gpr[j].part[2+i/2]<<16)|(y&0xffff);
        }
}

void do_vector_pack_swus(int j,int a,int b)   //mari
{
        signed int x,y;
	int i;
	for (i=0;i<4;i++)
...
void do_vector_pack_uhum(int j,int a,int b)   //mari
{
        unsigned int x,y;
	int i;
	for (i=0;i<8;i++)
        {   
	    x = chooseInterval(env->vector_gpr[a].part[i/2], 0xff, 16-(i%2)*16);
            env->vector_gpr[j].part[i/4] = (env->vector_gpr[j].part[i/4]<<8)|(x&0xff);
	    y = chooseInterval(env->vector_gpr[b].part[i/2], 0xff, 16-(i%2)*16);
            env->vector_gpr[j].part[2+i/4] = (env->vector_gpr[j].part[2+i/4]<<8)|(y&0xff);
        }
}

void do_vector_pack_uhus(int j,int a,int b)   //mari
{
        unsigned int x,y;
	int i;
	for (i=0;i<8;i++)
...
void do_vector_pack_uwum(int j,int a,int b)   //mari
{
        unsigned int x,y;
	int i;
	for (i=0;i<4;i++)
        {   
	    x = chooseInterval(env->vector_gpr[a].part[i], 0xffff, 0);
            env->vector_gpr[j].part[i/2] = (env->vector_gpr[j].part[i/2]<<16)|x;
	    y = chooseInterval(env->vector_gpr[b].part[i], 0xffff, 0);
            env->vector_gpr[j].part[2+i/2] = (env->vector_gpr[j].part[2+i/2]<<16)|y;
        }
}

void do_vector_pack_uwus(int j,int a,int b)   //mari
{
        unsigned int x,y;
	int i;
	for (i=0;i<4;i++)
        {   
	    x = chooseInterval(env->vector_gpr[a].part[i], 0xffffffff, 0);
            x = ClampU((unsigned)x, 0, 65535);
            env->vector_gpr[j].part[i/2] = (env->vector_gpr[j].part[i/2]<<16)|(x&0xffff);
	    y = chooseInterval(env->vector_gpr[b].part[i], 0xffffffff, 0);
	    y = ClampU((unsigned)y, 0, 65535);
            env->vector_gpr[j].part[2+i/2] = (env->vector_gpr[j].part[2+i/2]<<16)|(y&0xffff);
        }
}

unsigned int shiftingfirstfive(unsigned int m)    //mari
{
	int a,b,c,d;
	if(((m&0x80000000)>>31)==1) 
		a=0xff<<24;
	else if(((m&0x80000000)>>31)==0) 
		a=0x00000000;
	b=(m&0x7c000000)>>10;
	c=(m&0x03e00000)>>13;
	d=(m&0x001f0000)>>16;
...
unsigned int shiftingsecondfive(unsigned int m)    //mari
{
	int a,b,c,d;
	if(((m&0x00008000)>>15)==1) 
		a=0xff<<24;
	else if(((m&0x00008000)>>31)==0) 
		a=0x00000000;
	b=(m&0x00007c00)<<6;
	c=(m&0x000003e0)<<3;
	d=(m&0x00001f);
...
void do_vector_unpack_hpx(int j,int a,int b)  //mari
{
	env->vector_gpr[j].part[0]=shiftingfirstfive(env->vector_gpr[b].part[0]);
	env->vector_gpr[j].part[1]=shiftingsecondfive(env->vector_gpr[b].part[0]);
	env->vector_gpr[j].part[2]=shiftingfirstfive(env->vector_gpr[b].part[1]);
	env->vector_gpr[j].part[3]=shiftingsecondfive(env->vector_gpr[b].part[1]);
}
void do_vector_unpack_lpx(int j,int a,int b)  //mari
{
	env->vector_gpr[j].part[0]=shiftingfirstfive(env->vector_gpr[b].part[2]);
	env->vector_gpr[j].part[1]=shiftingsecondfive(env->vector_gpr[b].part[2]);
	env->vector_gpr[j].part[2]=shiftingfirstfive(env->vector_gpr[b].part[3]);
	env->vector_gpr[j].part[3]=shiftingsecondfive(env->vector_gpr[b].part[3]);
}

unsigned int get_halfword1(unsigned int m)
{
	int a,b;
	a=((m&0xff000000)>>24);
        a = (EXTS((unsigned)a,8)<<16);
	b=((m&0x00ff0000)>>16);
        b = EXTS((unsigned)b,8);
	m=a|b;
	return m;
}
unsigned int get_halfword2(unsigned int m)
{
	int a,b;
        a=((m&0x0000ff00)>>8);
        a = (EXTS((unsigned)a,8)<<16);
...
void do_vector_unpack_hsb(int j,int a,int b)  //mari
{
	env->vector_gpr[j].part[0]=get_halfword1(env->vector_gpr[b].part[0]);
	env->vector_gpr[j].part[1]=get_halfword2(env->vector_gpr[b].part[0]);
	env->vector_gpr[j].part[2]=get_halfword1(env->vector_gpr[b].part[1]);
	env->vector_gpr[j].part[3]=get_halfword2(env->vector_gpr[b].part[1]);
}
void do_vector_unpack_lsb(int j,int a,int b)  //mari
{
	env->vector_gpr[j].part[0]=get_halfword1(env->vector_gpr[b].part[2]);
	env->vector_gpr[j].part[1]=get_halfword2(env->vector_gpr[b].part[2]);
	env->vector_gpr[j].part[2]=get_halfword1(env->vector_gpr[b].part[3]);
	env->vector_gpr[j].part[3]=get_halfword2(env->vector_gpr[b].part[3]);
}

unsigned int get_word1(unsigned int m)
{
	int a;
	a=((m&0xffff0000)>>16);
        a = EXTS((unsigned)a,16);
	m=a;
	return m;
}
unsigned int get_word2(unsigned int m)
{
	int a;
	a=(m&0x0000ffff);
        a = EXTS((unsigned)a,16);
...
void do_vector_unpack_hsh(int j,int a,int b)  //mari
{
	env->vector_gpr[j].part[0]=get_word1(env->vector_gpr[b].part[0]);
	env->vector_gpr[j].part[1]=get_word2(env->vector_gpr[b].part[0]);
	env->vector_gpr[j].part[2]=get_word1(env->vector_gpr[b].part[1]);
	env->vector_gpr[j].part[3]=get_word2(env->vector_gpr[b].part[1]);
}
void do_vector_unpack_lsh(int j,int a,int b)  //mari
{
	env->vector_gpr[j].part[0]=get_word1(env->vector_gpr[b].part[2]);
	env->vector_gpr[j].part[1]=get_word2(env->vector_gpr[b].part[2]);
	env->vector_gpr[j].part[2]=get_word1(env->vector_gpr[b].part[3]);
	env->vector_gpr[j].part[3]=get_word2(env->vector_gpr[b].part[3]);
}

void do_vector_add_cuw(int vrt,int vra,int vrb)
{
    int i;
    unsigned int aop,bop,temp;
    for (i=0;i<4;i++){
        aop = env->vector_gpr[vra].part[i];
        bop = env->vector_gpr[vrb].part[i];
        temp = (0xffffffff-aop);
        //we have to do like it because the sum can't be stored in unsigned int
        if(bop>temp)
            env->vector_gpr[vrt].part[i] = 1;  
            else
                    env->vector_gpr[vrt].part[i] = 0;   
    } 
}

void do_printf_vector(int i)            //mari
{
     printf("vector gpr[%d],%x %x %x %x\n",i,env->vector_gpr[i].part[0],env->vector_gpr[i].part[1],env->vector_gpr[i].part[2],env->vector_gpr[i].part[3]);   
}
